...
http.createServer(function (req, res) {
	...
	if ( path.toString() === "/api/add" ){
		var out = a + b;
    	var txtq = "Result: " + a + "+" + b + "=" + out + "\n";
		return res.end(txtq);
	} else if ( path.toString() === "/api/sub"){
		var out = a - b;
    	var txtq = "Result: " + a + "-" + b + "=" + out + "\n";
		return res.end(txtq);
	} else if ( path.toString() === "/api/div"){
		var out = a / b;
    	var txtq = "Result: " + a + "/" + b + "=" + out + "\n";
		return res.end(txtq);
	} else if ( path.toString() === "/api/mul"){
		var out = a * b;
    	var txtq = "Result: " + a + "*" + b + "=" + out + "\n";
		return res.end(txtq);
	} else{
		res.write("The given  URL was: ");
		res.write(req.url);
		res.write("\n");
...
console.log('Server running, try http://0.0.0.0:<port>/api/add?a=1&b=2');
